...
if ($Setup) {
    ...
    try {
        ...
        Write-ColorText "✅ Netlify CLI installed" "Green"
    } catch {
        Write-ColorText "⚠️  Netlify CLI might need manual installation" "Yellow"
    }
    ...
    Write-Host ""
    Write-ColorText "✅ Setup completed! You can now use:" "Green"
...
if ($Deploy) {
    ...
    try {
        ...
        Write-ColorText "⚠️  Initial indexing had issues, continuing..." "Yellow"
    }
    
    Write-ColorText "🚀 Fast deploying to Netlify (no build)..." "Cyan"
    try {
        # Build yapmadan direkt deploy - çok daha hızlı ve build minutes tasarrufu
        ...
        Write-ColorText "🔧 Use admin panel to manually trigger or check status" "Yellow"
...
if ($Status) {
    ...
    Write-ColorText "🔍 Testing admin endpoint..." "Cyan"
    try {
        $response = Invoke-RestMethod -Uri "https://orderstake.netlify.app/.netlify/functions/indexer-admin?action=status" -Method GET
        Write-ColorText "✅ Admin endpoint is working" "Green"
        Write-ColorText "📈 Stats: $($response.stats.totalPools) pools, $($response.stats.totalEvents) events" "White"
    } catch {
        Write-ColorText "⚠️  Admin endpoint test failed (might be normal if not deployed yet)" "Yellow"
    }
